...
Begin with [lesson 1](/ai-now/start-a-grounded-task). Each lesson prepares its own sample memories and sources, so no model setup or manual import is needed before opening the simulation. Refreshing resets the practice. On mobile, you can read the steps; use desktop for the interactive exercise.
