...
- Every course overview with an available first lesson must place a `.course-cta` link immediately after `.course-intro`, labeled **Start lesson 1** / **开始第一课**. Link to the locale-specific first lesson without `.mdx`; lesson cards and inline links do not replace this primary entry. Verify the button on desktop/mobile in both locales. For AI Now, use `/ai-now/start-a-grounded-task` and `/zh/ai-now/start-a-grounded-task` without `#simulation`, preserving the first lesson's manual Playground start.
